...
const setting = JSON.parse(fs.readFileSync('./lib/configuração.json'))
prefix = configuração.prefix
blocked = []
fake = 'TermuxBot '
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color('☝️'))
	})

	fs.existsSync('./qr.json') && client.loadAuthInfo('./qr.json')
	client.on('connecting', () => {
		start('2', 'Conectando bot')
	})
	client.on('open', () => {
		success('2', 'Pronto bot on :)')
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá👋 @${num.split('@')[0]}\nSeja bemvindo ao grupo *${mdata.subject}* leia as regras para não ser banido do grupo`
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`${configuração.NumeroDono}@s.whatsapp.net`]
			...
			function addMetadata(packname, author) {	
				if (!packname) packname = 'TermuxBot'; if (!author) author = 'Bot';	
...
case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Você é ativo?')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('️Ativar com sucesso o recurso de boas-vindas neste grupo')
					} else if (Number(args[0]) === 0) {
